...
export interface EvalRunResult {
  perTask: ScoreResult[];
  /** Mean score over deterministically-scored (non-pending) tasks; null if none. */
...
export async function scoreRollouts(
  ...
  const scored = perTask.filter((r) => !r.pending);
  const aggregate =
    scored.length > 0 ? scored.reduce((sum, r) => sum + r.score, 0) / scored.length : null;

  return { perTask, aggregate, pendingJudgments };
}
